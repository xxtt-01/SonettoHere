...
![提供商管理](images/%E6%8F%90%E4%BE%9B%E5%95%86%E7%AE%A1%E7%90%86.png)
...
内置 37 个 Built-in Tool，涵盖日常工具链：

| 领域 | Tools | 需要配置 |
|------|-------|----------|
| **Todo** | 添加/列出/完成/取消完成/删除/更新/查询任务、列出项目/分区/标签 | `TODOIST_API_TOKEN` |
| **地图** | 周边搜索、地址编码、公交路线、骑行路线、模糊地址 | `AMAP_API_KEY` |
| **网络** | 天气查询、节假日日历、图片理解、Tavily 搜索、Tavily 网页提取 | `UAPIS_API_KEY` + `TAVILY_API_KEY` + `ZHIPUAI_API_KEY` |
| **文件** | 读取/写入/编辑、删除/重命名/创建目录、目录列表/文件搜索 | — |
| **任务追踪** | 无状态任务清单追踪 | — |
| **系统** | Python 脚本执行 | — |
...
![工具调用](images/%E5%B7%A5%E5%85%B7%E8%B0%83%E7%94%A8.png)
...
支持通过浏览器 Web Speech API 进行语音输入，将语音实时转为文字。

- **触发**：长按输入框旁的麦克风按钮，或按住 `Space` 键
- **停止**：松开按钮或 Space 键，语音自动识别为文字填入输入框
- **兼容性**：依赖浏览器原生 Speech Recognition 能力（推荐 Chrome/Edge）

适用于打字不便时快速输入等场景。

## 视觉模式

开启图像认知后，发送消息时附带引用的本地文件图片将以 base64 多模态内容直接注入主 LLM 的消息，AI 可以直接"看懂"图片内容。

- **切换**：点击输入框旁的「图像认知」按钮（相片图标）
- **开启时**：消息中引用的图片会以多模态消息格式发送给 LLM
- **依赖**：当前使用的主 LLM 需支持多模态视觉能力

![图像认知](images/%E5%9B%BE%E5%83%8F%E8%AE%A4%E7%9F%A5.png)

*[谨向知名画师雾见冷水老师致意](https://space.bilibili.com/491189)*

适用于识别截图、解读图表、分析照片等需要视觉理解的场景。

LLM在提供商页面被加入时将自动核验图像多模态能力并由模型名索引形式记录在本地。核验未通过的LLM将无法启用图像认知模式。

## 私密模式

私密模式下，当前对话**不会被保存到长期记忆本地存储**。关闭后恢复正常保存。

- **切换**：点击输入框的「私密」按钮，或按 `Ctrl + K`
- **开启时**：对话内容仅存在于当前会话中，不参与长期记忆
- **关闭时**：对话内容会被 AI 总结并写入 `memory.yaml`，形成长期记忆

适用于临时咨询、隐私话题等不需要被记住的对话场景。

**会话启用私密模式后，在同一会话内请勿将其关闭，否则后续触发长期记忆存储时可访问到私密模式下的对话内容。**
